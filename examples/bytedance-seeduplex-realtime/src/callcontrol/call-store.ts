...
import { createSeeduplexRealtimeBridge } from '../providers/seeduplex-realtime.ts';
import type { SeeduplexBridgeHandle, SeeduplexToolWire } from '../providers/seeduplex-realtime.ts';
...
export function createCallStore(
    ...
    const participantState = new Map<number, { bridge: SeeduplexBridgeHandle; fileLog: FileCallLogger }>();
    ...
    const handleParticipantConnected = (participant: Participant) => {
        ...
        logger.event('call_start', { participantId: participant.id, mode: 'bytedance-seeduplex-realtime' });
        const fileLog = getCallLogger(participant.id, { tag: 'Seeduplex' });
        ...
        const allTools: SeeduplexToolWire[] = [
            ...localToolDefs.map((t): SeeduplexToolWire => ({
                type: 'function',
                name: t.function.name,
                description: t.function.description ?? '',
                parameters: (t.function.parameters ?? { type: 'object', properties: {} }) as Record<string, unknown>,
            })),
            ...(mcpToolDefs ?? []).map((t): SeeduplexToolWire => ({
                type: 'function',
                name: t.name,
                description: t.description,
                parameters: t.parameters,
            })),
            ...(customMcpRouter?.toolDefs ?? []).map((t): SeeduplexToolWire => ({
                type: 'function',
                name: t.name,
                description: t.description,
                parameters: t.parameters,
        ...
        const bridge = createSeeduplexRealtimeBridge(
            participant,
            {
                volcApiKey: appConfig.volcApiKey,
                volcAppId: appConfig.volcAppId,
                model: appConfig.realtimeModel ?? '1.2.6.0',
                voice: profile?.voice ?? appConfig.realtimeVoice ?? 'zh_female_vv_jupiter_bigtts',
                instructions,
                localTools: allTools,
                greeting,
    ...
    console.log(chalk.cyan('[CallStore] initialized (Seeduplex 3.0 duplex realtime)'));
}
